        ...
        .then(members => {
            ...
            for (mem of members) {
                if (mem[1]._roles.length < 1) {
                    ...
                    let daysInServer = Math.floor(((now - new Date(mem[1].joinedTimestamp)) / 86400000)) //?24(hours) * 60(minutes) * 60(seconds) * 1000(milliseconds)
                    if (daysInServer > daysToKick) {
                        let name = `<@${mem[1].user.id}>`
                        list.push(`${name} - *Time in server: ${daysInServer} days*`)
                    }
                }
            }
            if (list.length) {
