...
function sendReminder() {
	...
	const embed = new MessageEmbed()
		...
		.setFooter(
			`${date.getMonth()}/${date.getDate()}/${date.getFullYear()}   ${pstTimestamp}   ${estTimestamp}`
		)
		.setColor(0xff0000)
		.setThumbnail(
	...
	client.channels.fetch(config.announceChannelID).then((channel) => {
		channel.send('Daily Reports are available! @everyone')
		channel.send(embed)
	})
}
